...
int main() {
    ...
    for (int outerIt = 1; outerIt <13; outerIt++) {
      ...
      //  printMat(Fs, "Fs");
        ...
        forAllInternalUCVs(p) {
           // AEp[i][j]     += -density * dy * dy / (APu[i + 1][j] * IFi + APu[i][j] * (1 - IFi));
            ...
            AWp[i + 1][j] += -density * dy * dy * ( IFi / APu[i + 1][j] + (1 - IFi)/APu[i][j]);
        }
        // pressure corretion equation 
        forAllInternalVCVs(p) {
            ANp[i][j]     += -density * dx * dx * ( IFi/ APv[i][j + 1] + (1 - IFi) /APv[i][j]  );
            ASp[i][j + 1] += -density * dx * dx * ( IFi/ APv[i][j + 1] + (1 - IFi) /APv[i][j] );
        }
        forAllInternal(p) {
      ...
      //  printMat(pp, "pp");
        ...
        forAllInternal(U) {
            ...
            UU[i][j] = UU[i][j] + (URF * dy / APu[i][j]) * (PPw - PPe);
            VV[i][j] = VV[i][j] + (URF * dx / APv[i][j]) * (PPs - PPn);
        }
